...
function insert( $data, $data1, $idSensor, $mimosaTxPhyTotal, $mimosaRxPhyTotal, $statusAlert, $StErro ) {
	$timearq = date("H-i-s");
	$arq = $idSensor . "_" . $timearq;
	// |$data|$data1|$idSensor|$valor1|$valor2|$valor3|$statusAlert|$StErro|
	exec("echo '|$data|$data1|$idSensor|$mimosaTxPhyTotal|$mimosaRxPhyTotal||$statusAlert|$StErro|' > /var/www/html/ram/coletas/valores/$arq");
}
...
if($txphy1 && $txphy2 && $rxphy1 && $rxphy2) {
	$tx = $txphy1 + $txphy2 + $txphy3 + $txphy4;
	$rx = $rxphy1 + $rxphy2 + $rxphy3 + $rxphy4;
