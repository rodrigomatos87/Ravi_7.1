...
if($vsnmp == 1) {
    $voltagem = snmpget("{$ip}:{$porta}", $snmp, "1.3.6.1.4.1.17095.1.3.12.0", 1000000, 30);
}else if($vsnmp == 2) {
    $voltagem = snmp2_get("{$ip}:{$porta}", $snmp, "1.3.6.1.4.1.17095.1.3.12.0", 1000000, 30);
}else if($vsnmp == 3) {
	$voltagem = snmp3_get("{$ip}:{$porta}", $snmp, $nivelsegsnmp, $protocoloauthsnmp, $authsnmp, $protocolocripsnmp, $criptosnmp, "1.3.6.1.4.1.17095.1.3.12.0", 1000000, 30);
}

if($voltagem) {
	$voltagem = sanitizeSNMP($voltagem) / 10;
	if($voltagem == 0) {
		if($StErro >= $falhas) {
			if($alertar == 1) {
				$statusAlert = 3;
