...
if($vsnmp == 1) {
	$voltagem = sanitizeString(snmpget("{$ip}:{$porta}", $snmp, "1.3.6.1.4.1.5875.800.3.9.3.4.1.9.{$valor}", 1000000, 30));
}else if($vsnmp == 2) {
	$voltagem = sanitizeString(snmp2_get("{$ip}:{$porta}", $snmp, "1.3.6.1.4.1.5875.800.3.9.3.4.1.9.{$valor}", 1000000, 30));
}else if($vsnmp == 3) {
	$voltagem = sanitizeString(snmp3_get("{$ip}:{$porta}", $snmp, $nivelsegsnmp, $protocoloauthsnmp, $authsnmp, $protocolocripsnmp, $criptosnmp, "1.3.6.1.4.1.5875.800.3.9.3.4.1.9.{$valor}", 1000000, 30));
}

if(isset($voltagem) && $voltagem != "") {
	$voltagem = $voltagem / 100;
	if($media1 >= 2 && $media1 != "" && isset($maxPer)) {
