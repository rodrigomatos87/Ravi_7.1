...
function sanitizeString($string) {
    $what = array( 'INTEGER:', ' ', 'mA', 'STRING:', '"' );
    $by   = array( '','','','','' );
    return str_replace($what, $by, $string);
}

if($vsnmp == 1) {
	$bias = sanitizeString(snmpget("{$ip}:{$porta}", $snmp, "1.3.6.1.4.1.37950.1.1.5.10.13.1.1.4.{$valor}", 1000000, 30));
}else if($vsnmp == 2) {
	$bias = sanitizeString(snmp2_get("{$ip}:{$porta}", $snmp, "1.3.6.1.4.1.37950.1.1.5.10.13.1.1.4.{$valor}", 1000000, 30));
}else if($vsnmp == 3) {
	$bias = sanitizeString(snmp3_get("{$ip}:{$porta}", $snmp, $nivelsegsnmp, $protocoloauthsnmp, $authsnmp, $protocolocripsnmp, $criptosnmp, "1.3.6.1.4.1.37950.1.1.5.10.13.1.1.4.{$valor}", 1000000, 30));
}
