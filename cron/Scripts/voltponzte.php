...
if($vsnmp == 1) {
	$voltagem = sanitizeString(snmpget("{$ip}:{$porta}", $snmp, "1.3.6.1.4.1.3902.1015.3.1.13.1.10.{$valor}", 1000000, 30));
}else if($vsnmp == 2) {
	$voltagem = sanitizeString(snmp2_get("{$ip}:{$porta}", $snmp, "1.3.6.1.4.1.3902.1015.3.1.13.1.10.{$valor}", 1000000, 30));
}else if($vsnmp == 3) {
	$voltagem = sanitizeString(snmp3_get("{$ip}:{$porta}", $snmp, $nivelsegsnmp, $protocoloauthsnmp, $authsnmp, $protocolocripsnmp, $criptosnmp, "1.3.6.1.4.1.3902.1015.3.1.13.1.10.{$valor}", 1000000, 30));
}

if(isset($voltagem) && $voltagem != "") {
    $voltagem = $voltagem / 1000;
    $voltagem = number_format($voltagem, 1, '.', '');
	if($media1 >= 2 && isset($maxPer)) {
