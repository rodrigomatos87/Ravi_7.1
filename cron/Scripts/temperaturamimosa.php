...
function insert( $data, $data1, $idSensor, $tempmimo, $statusAlert, $StErro ) {
	$timearq = date("H-i-s");
	$arq = $idSensor . "_" . $timearq;
	// |$data|$data1|$idSensor|$valor1|$valor2|$valor3|$statusAlert|$StErro|
	exec("echo '|$data|$data1|$idSensor|$tempmimo|||$statusAlert|$StErro|' > /var/www/html/ram/coletas/valores/$arq");
}
...
if($vsnmp == 1) {
	$temperatura = sanitizeSNMP(snmpget("{$ip}:{$porta}", $snmp, "1.3.6.1.4.1.43356.2.1.2.1.8.0", 1000000, 30));
}else if($vsnmp == 2) {
	$temperatura = sanitizeSNMP(snmp2_get("{$ip}:{$porta}", $snmp, "1.3.6.1.4.1.43356.2.1.2.1.8.0", 1000000, 30));
}else if($vsnmp == 3) {
	$temperatura = sanitizeSNMP(snmp3_get("{$ip}:{$porta}", $snmp, $nivelsegsnmp, $protocoloauthsnmp, $authsnmp, $protocolocripsnmp, $criptosnmp, "1.3.6.1.4.1.43356.2.1.2.1.8.0", 1000000, 30));
}
