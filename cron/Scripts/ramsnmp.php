...
if($vsnmp == 1) {
	$totaR = sanitizeSNMP(snmpget("{$ip}:{$porta}", $snmp, "1.3.6.1.4.1.2021.4.5.0", 1000000, 30));
}else if($vsnmp == 2) {
	$totaR = sanitizeSNMP(snmp2_get("{$ip}:{$porta}", $snmp, "1.3.6.1.4.1.2021.4.5.0", 1000000, 30));
}else if($vsnmp == 3) {
	$totaR = sanitizeSNMP(snmp3_get("{$ip}:{$porta}", $snmp, $nivelsegsnmp, $protocoloauthsnmp, $authsnmp, $protocolocripsnmp, $criptosnmp, "1.3.6.1.4.1.2021.4.5.0", 1000000, 30));
}

if($totaR) {
	if($vsnmp == 1) {
		$livreR = sanitizeSNMP(snmpget("{$ip}:{$porta}", $snmp, "1.3.6.1.4.1.2021.4.6.0", 1000000, 30));
	}else if($vsnmp == 2) {
		$livreR = sanitizeSNMP(snmp2_get("{$ip}:{$porta}", $snmp, "1.3.6.1.4.1.2021.4.6.0", 1000000, 30));
	}else if($vsnmp == 3) {
		$livreR = sanitizeSNMP(snmp3_get("{$ip}:{$porta}", $snmp, $nivelsegsnmp, $protocoloauthsnmp, $authsnmp, $protocolocripsnmp, $criptosnmp, "1.3.6.1.4.1.2021.4.6.0", 1000000, 30));
	}
	$usoR = $totaR - $livreR;
